...
class StepRepository @Inject constructor(private val apiService: StepService) {
    ...
    suspend fun createStep(id: Int, stepModel: StepRequestBody, token: String): Result<Throwable> = NetworkResultWrapper.wrapWithResult {  apiService.createStep(id = id, stepModel = stepModel, token = token) }
